...
public enum EnumStairsHalf implements NMSEnum {
    ...
    public static final Class<?> enumStairHalfClass = JReflection.getReflectionClass("net.minecraft." + (JVersion.getServerVersion().isNewerOrEqual(JVersion.v1_17) ? "world.level.block.state.properties" : "server." + JReflection.getNMSVersion()) + "." + (JVersion.getServerVersion().isNewerOrEqual(JVersion.v1_13) ? "BlockPropertyHalf" : "BlockStairs$EnumHalf"));

    @Override
    public Object getNMSObject() {
        if(JVersion.getServerVersion().isLowerOrEqual(JVersion.v1_7))
            throw new NMSException("EnumStairsHalf is not supported in versions below 1.7!");
        return JReflection.getEnum(this.ordinal(), enumStairHalfClass);
    }
