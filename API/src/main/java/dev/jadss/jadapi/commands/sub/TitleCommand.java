...
public class TitleCommand {
    ...
    public TitleCommand(CommandSender sender, String[] args) {
        if(sender instanceof ConsoleCommandSender || sender.hasPermission("JadAPI.sendTitle")) {
            ...
            if(args[1].equalsIgnoreCase("*")) {
                for(JPlayer p : JPlayer.getJPlayers())
                    p.sendTitle(ChatColor.translateAlternateColorCodes('&', message), null);
            } else {
                Player player = Bukkit.getPlayer(args[1]);
                if(player != null && player.isOnline()) {
                    new JPlayer(player).sendTitle(ChatColor.translateAlternateColorCodes('&', message), null);
                } else {
